...
export default function App() {
  ...
  const fetchReports = useCallback(async () => {
    try {
      ...
      const reports = json.data || json.reports || json;

      const mapped = reports
        .filter(r => r.report_status === "RESOLVED" || r.report_status === "IN_PROGRESS")
        .map((r) => {
          const { date, time } = formatDate(r.created_at);
          return {
            id: r.id,
            type: typeFromBackend(r.hazard_type),
            ...
            date: date,
            loc: r.address,
            statusMsg: r.report_status === "RESOLVED" ? "Issue resolved" :
                       r.report_status === "IN_PROGRESS" ? "Work in progress" : "Pending",
            img: r.image_url,
            aiConf: "95%",
            aiStatus: "Verified",
